...
#define studentloginpage "/root/website/files/pages/studentloginpage.html"
...
static enum erno db_lookup(char **input,int cfd)
{
	...
	command="select * from students where stud_id=$1 and stud_password=$2";
...
static enum erno send_page(char *input , unsigned char *nonce,int sockfd)
{
	...
	if(gethtmlcontent(studentloginpage,&buffer,size)!=GETHTML_OK)
	{
...
int main(int argc , char* argv[])
{
	...
	if(cfd!=-1)
	{
		...
		{
			flow=1;
			logg("Failed to recieve input(loginworker)\n");
		}
