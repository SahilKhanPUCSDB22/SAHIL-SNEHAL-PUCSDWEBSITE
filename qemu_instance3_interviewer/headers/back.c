...
//ERRORS
enum erno{GEN_OK,GEN_NA,INPUT_OK,INPUT_NA,VALUES_OK,VALUES_NA,INSERT_OK,INSERT_NA,UID_NA,PASS_NA,DATA_F,SESSION_OK,SESSION_NA,PAGE_OK,PAGE_NA,UPDATE_NA,UPDATE_OK,USER_OK,USER_NA,GETHTML_OK,SENDHTML_OK,ATT_OK,ATT_NA,REMOVE_NA,REMOVE_OK,STREAM_OK,STREAM_NA,FILE_OK,FILE_NA};
...
int connect_db()
{
	con = PQconnectdb("host=192.168.1.10 dbname=pucsd_hackerrank user=postgres password=1234 port=5432 connect_timeout=4");
...
static enum erno get_stream( int cfd , char ** buffer )
{
	*(buffer) = malloc (sizeof(char)*(CAPACITY*2));

	int i = CAPACITY , current = CAPACITY*2 , recieved=0;

	while(i == CAPACITY)
	{
		if( (i = recv( cfd, *(buffer)+recieved , CAPACITY , 0 ) ) <=0 )
		{
			logg("Failed to recieved qdata\n");
		...
		if(recieved == current)
		{
			...
			*buffer = realloc(*buffer,sizeof(char)*current);
		}
	}
