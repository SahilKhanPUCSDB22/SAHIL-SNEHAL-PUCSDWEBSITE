...
static enum erno send_input(struct kreq *r){
	...
	int size = SESSION_LEN + TESTID + UTYPE + QNO + FNAME_LEN + FSIZE_LEN+  KEY__MAX;
		...
		else
		{
			...
			{	if(send_file(&r)!=PAGE_OK)
				...
				else
				{
					...
					{
						khttp_puts(&r,"Failed to Upload File.Response Reciept Failure From Server\n");
					}
				}
			}
		}
