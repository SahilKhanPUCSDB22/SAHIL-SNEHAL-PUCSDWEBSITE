...
#define link "<a href=\"#\" onclick=\"viewanswer("
#define linkMid ")\">"
#define linkEnd "</a>"
#define Heading "<h1 style=\"background-color:lightgreen\" >Student List</h1>\n"
...
static enum erno view(char *input,char *filename)
{
	...
	command="select s1.studid,s2.stud_fname,s1.submitdate,s1.submittime,s1.status,s1.marks from studenttest s1 join students s2 on s1.studid=s2.stud_id where testid=$1";
	...
	else
	{
		...
		while(i<rows)
		{
			...
			//submit date
			wp(columnstart);
			wp(PQgetvalue(res,i,2));
			wp(columnEnd);
			//submit time
			wp(columnstart);
			wp(PQgetvalue(res,i,3));
			wp(columnEnd);
			//status
			wp(columnstart);
			wp(PQgetvalue(res,i,4));
			wp(columnEnd);
			//marks
			wp(columnstart);
			wp(PQgetvalue(res,i,5));
			wp(columnEnd);
...
static enum erno send_page(int cfd ,char**input,char *filename)
{
	...
	contentsize[0]=total+mylen(input[0])+mylen(input[1])+1;
...
int main(int argc , char* argv[])
{
	...
	int streamlen= SESSION_LEN + TESTID + INPUT_MAX , size[]={SESSION_LEN,TESTID},cfd;
	...
	if(cfd!=-1)
	{
		...
		if(flow==0 && (view(input[1],filename)!=GEN_OK))
		{
			flow=1;
		}
