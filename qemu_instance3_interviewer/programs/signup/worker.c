...
#define checkuser "select * from interviewers where interviewers_id=$1"
#define checkcompany "select * from companies where company_id=$1"
#define userexists "UserId already present.Please try a different one"
#define companyexist "CompanyId does not exist.Please try again"
#define insertuser  "insert into interviewers values($1,$2,$3,$4,$5,$6)"
...
static enum erno checkuserid(char * user,char *company,int cfd)
{
	char *params[1];
	params[0]=user;
	...
	res= PQexecParams(con,(char*)checkcompany,1,NULL,(const char* const*)params,NULL,NULL,0);
...
{
	int inpsize = USERID+PASSWORD+SFNAME+SLNAME+ROLE+COMPANY,cfd=myatoi(argv[1]);

        int size[]={USERID+PASSWORD+SFNAME+SLNAME+ROLE+COMPANY};	
