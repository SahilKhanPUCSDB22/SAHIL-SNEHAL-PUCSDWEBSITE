...
static enum erno mcqdisplay(char **input,char *filename,int *srno,char *userdetail)
{

	int rows,row1,i=0; total=0; 

	char *command,sr,*params[]={input[1],userdetail};
	
	tp=fopen(filename,"w+");
	
	wp(fileStart);
	
	command="select ques from mquestion where qno in (select qno from manswer where testid=$1 and studid=$2 order by qno asc) and testid=$1";
	...
	{
		command="select optext from options o join manswer m on m.qno=o.qno and m.answer=o.ono where m.testid=$1 and m.studid=$2 order by m.qno asc";
...
static enum erno desdisplay(char **input,char *filename,int *srno,char* userdetail)
{
	int rows,row1,i=0; 
	...
	command="select ques from dquestion where qno in (select qno from danswer where testid=$1 and studid=$2 order by qno asc) where testid=$1";
...
static enum erno filedisplay(char **input,char *filename,int *srno,char *userdetail)
{
	int rows,row1,i=0; 
	
	char *command,sr,*params[]={input[1],userdetail};
	
	command="select ques from fquestion where qno in(select qno from fanswer where testid=$1 and studid=$2 order by qno asc) where testid=$1";
...
int main(int argc , char* argv[])
{
	...
	int streamlen= SESSION_LEN + TESTID + INPUT_MAX , size[]={SESSION_LEN,TESTID},cfd,srno=1;
	...
	if(cfd!=-1)
	{
		...
		if(flow==0 && (mcqdisplay(input,filename,&srno,userdetails[0])!=GEN_OK))
		{
			flow=1;
		}
		else
		...
		if(flow==0 && (desdisplay(input,filename,&srno,userdetails[0]))!=GEN_OK)
		{
			flow=1;
		}
		if(flow==0 && (filedisplay(input,filename,&srno,userdetails[0]))!=GEN_OK)
		{
			flow=1;
		}
